	...
	*/
    ...
    //FINE modifiche PALAGIANO MARCELLO
	    //inizio modifiche Giovanni Lentini
            ...
            vocabolario.inserisci("fabbricante",108);
...
// fine modifiche CHIARAPPA ROSA
    ...
    vocabolario.inserisci("sforzo", 99);
    // (fine integrazione Spina)

    //inizio modifiche Bagordo Simone
    vocabolario.inserisci("teletrasporta", 135);
    //fine modifiche Bagordo Simone

    //INIZIO INTEGRAZIONE LORENZO BALDARI
    vocabolario.inserisci("entra nella",74); //74 OK
    vocabolario.inserisci("mostra",18);
    //FINE INTEGRAZIONE LORENZO BALDARI
    ...
    */
    ...
    azioni.inserisci(3500800086, 125); //usa slotmachine
    ...
    // (inizio integrazione Spina)
    azioni.inserisci(990000, 147); // info su sforzo fisico (Luogo: 0000 (ovunque), Verbo: 0099, Oggetto: 0000 (nessuno)
    // (fine integrazione Spina)
        ...
        //INIZIO MODIFICHE PIERLUIGI BEMPORTATO
        azioni.inserisci(4400300108,148);//parla con fabbricante
        //FINE MODIFICHE PIERLUIGI BEMPORTATO

        //inizio modifiche Simone Bagordo
        azioni.inserisci(1350000, 149);
        //fine modifiche Simone Bagordo

        //INIZIO INTEGRAZIONE LORENZO BALDARI
        azioni.inserisci(1800740018,150); //AZIONE “ENTRA NELLA MOSTRA”
        //FINE INTEGRAZIONE LORENZO BALDARI
...
//inizio modifiche Lentini Giovanni
	...
	oggetti.inserisci(Oggetto("un elaboratore bonus", 104,-43));
	 ...
	 //INIZIO MODIFICHE PIERLUIGI BEMPORTATO
	 //oggetti.inserisci(Oggetto("un fabbricante",108,-44));
	 //FINE MODIFICHE PIERLUIGI BEMPORTATO
...
// (inizio integrazione Spina)
void Astro::azione_147() {

    interfaccia.scrivi("***************calcolo sforzo********************");
    system("cls");
    sforzo.StampaPassi();
    interfaccia.scrivi("*************************************************");
}
...
if(numero_estrazione == numero_scelto){ controllo_numeri = true;}
...
//INIZIO MODIFICHE PIERLUIGI BEMPORTATO
void Astro::azione_148()
{
    Fabbrica fabbrica;
    int costruisci;

    system("cls");
    cout<<"\n\n\n\t\t\t\t\t\t\t\tBENVENUTO IN FABBRICA!\n\n";;
    cout<<"\t\t\t\t\t\tQui potrai guadagnare denaro costruendo degli oggetti,\n";
    cout<<"\t\t\t\t\t\tinserendo IN ORDINE i vari materiali necessari!\n\n\n";

    do{

        cout<<"\t\t\t\t\t\tCosa desideri costruire? Scrivi il numero!\n\n";
        cout<<"\t\t\t\t\t\t-1 Collana\t\tRicompensa: 2 Euro\n";
        cout<<"\t\t\t\t\t\t-2 Tavolo\t\tRicompensa: 10 Euro\n";
        cout<<"\t\t\t\t\t\t-3 Porta in metallo\tRicompensa: 20 Euro\n";
        cout<<"\t\t\t\t\t\t-4 Computer\t\tRicompensa: 50 Euro\n";
        cout<<"\t\t\t\t\t\t-5 Missile\t\tRicompensa: 100 Euro\n";
        cout<<"\t\t\t\t\t\t-0 Esci\n";
        cout<<"\n\t\t\t\t\t\tScelta: ";
        cin>>costruisci;

        if(costruisci <0 || costruisci >5)
        {
             cout<<"\n\n\t\t\t\t\t\t\tL'OGGETTO SCELTO NON E' DISPONIBILE, SCEGLINE UN ALTRO!\n\n";
        }
        else
        {
            ...
            if(!trovato)
            {
               ...
               interfaccia.scrivi("\n\n\t\t\t\t\t\t\tNon puoi ricevere denaro se non hai il portafoglio!\n\n");

            }
            else if (trovato){

                float guadagno = fabbrica.scelta(costruisci);
                ...
                cout<<"\n\n\t\t\t\t\t\t\tORA I TUOI CONTANTI NEL PORTAFOGLIO SONO: "<<portafoglio.get_contanti()<<"\n\n\n";
...
//AZIONE TELETRASPORTA
//inizio modifiche Simone Bagordo
void Astro::azione_149(){
	string luogo_teletrasporta;
	int luogo_teletrasporta_int;
    ...
    do{
        luogo_teletrasporta = interfaccia.leggi_stringa("In quale luogo desideri essere teletrasportato?");
           ...
           for (int i = 0; i < luogo_teletrasporta.length(); i++)
            ...
            if (luogo_teletrasporta != "indietro" && ( luogo_teletrasporta_int < 1 || luogo_teletrasporta_int > 42)){
                cout << "Non ho capito!\n";
            }

    }while(luogo_teletrasporta != "indietro" && ( luogo_teletrasporta_int < 1 || luogo_teletrasporta_int > 44));


    if (luogo_teletrasporta != "indietro"){

            if(luogo_attuale == luogo_teletrasporta_int){
                cout << "Ti trovi gia' nel luogo specificato!\n";
            }
            else{

            if ( teletrasporta.pilavuota()){ //Initial place where teleportation was used
                teletrasporta.inpila(luogo_attuale);
            }

            luogo_attuale = luogo_teletrasporta_int;
            teletrasporta.inpila(luogo_teletrasporta_int);
            }
    }
    else if ( luogo_teletrasporta == "indietro" && !teletrasporta.pilavuota()){
            teletrasporta.fuoripila();
            if (!teletrasporta.pilavuota() ){
            luogo_attuale = teletrasporta.leggipila();
            }
            else{
               cout << "Ti trovi al punto di partenza del teletrasporto!\n";
            }
    }
    else{
            cout << "Ti trovi al punto di partenza del teletrasporto!\n";
...
void Astro::azione_150()
{
    system("cls");
    //system("clear"); //linux os

    int num_persone=(rand()+1)%11;
    bool uscita_mostra=false;
    bool uscita_coda=false;
    string risposta_attesa; //input per la risposta
    int risposta_documento; //input per la risposta al documento
    ...
    Mostra mostra;
    Lista<typename Grafo<StanzaMostra,unsigned char>::nodo> listastanze;


    while (!uscita_mostra)
    {
        copiaLista(listastanze,mostra.getstanzeadiacenti());
        interfaccia.scrivi(" -----------------------------------------------------------------------------");
        cout<<endl<<mostra.leggiStanza(mostra.getstanzacorrente()).gettitolo()<<endl<<endl;
        cout<<mostra.leggiStanza(mostra.getstanzacorrente()).getdescrizione()<<endl<<endl;
        interfaccia.scrivi(" -----------------------------------------------------------------------------");

        int risposta;

        cout<<"Scegliere la stanza nella quale spostarsi:"<<endl;
        typename Lista<Grafo<StanzaMostra, unsigned char>::nodo>::posizione indice = listastanze.primolista();
        int i = 0;
        while(!listastanze.finelista(indice))
            {
                cout<< i<<" - "<<mostra.leggiStanza(listastanze.leggilista(indice)).gettitolo()<<endl;
                i++;
                indice = listastanze.succlista(indice);
            }
        cout<<i<<" - Uscita dalla Mostra"<<endl;
        ...
        else
        {
            ...
            if((risposta>=0)&&(risposta<=i))
            {
                ...
                while(!(listastanze.finelista(indice)))
                {
                    if(j == risposta)
                    {
                        mostra.setstanzacorrente(listastanze.leggilista(indice));
                        indice = listastanze.succlista(indice);
                        j++;
                    }
                    else
            ...
            {
                mostra.setstanzacorrente(mostra.getiniziale());
                uscita_mostra = true;
                system("cls");
                //system("clear"); //linux os
                fflush(stdin);
	...
	//inizio modifiche Lentini Giovanni
		  ...
		  break;
           //fine modifiche Lentini Giovanni
            ...
            case 150:
                azione_150();  //INTEGRAZIONE LORENZO BALDARI
            break;
            //FINE INTEGRAZIONE LORENZO BALDARI
    default:
        eseguito = false;
