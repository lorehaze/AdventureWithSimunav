...
void Gioco::Aggiorna_Tempo_bonus()       // Museo
{
    ...
    bool Gioco::biglietto_mostra() //funzione ausiliare per controllare la disponibilita' del portafoglio
    ...
    if(trovato==true)
    {
        ...
        if (saldo_p>=10)
        {
            ...
            do
            {
                ...
                interfaccia.scrivi("Premi invio per continuare...");
                cin>>a;
            }
            while(a==" ");
            acquisto=true;
...
bool Gioco::direzioni(Mappa &M) {
	...
	if (a == 0){
		...
		if (((autobus.get_inUso()==true)||(automobile.get_inUso()==true)) && (a<38 && a!=13 && a!=41)){
            ...
            sforzo.ins_contapassi(luogo_prec, luogo_succ);
...
void Gioco::azione_46()
{
    ...
    if(oggetti.get_oggetto(12).get_luogo() != 0 )
        interfaccia.scrivi("Non ce l'hai.");
    else {ifstream check ("codici.txt");
             if (!check)
